...
export default function Inventory() {
  ...
  const formatWeight = (grams) => {
    ...
    if (units === "imperial") {
      const lbs = g * 0.00220462262185; // grams to pounds
  ...
  function addItem(e) {
    e.preventDefault();
    if (!name) return;
    // weight input may be kg or lb depending on units; store internally as grams
    const raw = Number(weight) || 0;
    const grams =
      units === "imperial"
        ? Math.round(raw * 453.59237) // lb -> grams
        : Math.round(raw * 1000); // kg -> grams
    const it = {
  ...
  function startEdit(it) {
    ...
    setEditValues({
      name: it.name || "",
      // convert stored grams to selected units string for editing
      weight: it.weight
        ? units === "imperial"
  ...
  return (
    <>
      {/* Full-width banner for Inventory page */}
      <div
        className='w-full relative bg-center bg-cover h-44 md:h-64 lg:h-80'
        style={{
          backgroundImage:
            "url('/images/holly-mandarich-7MrXw_o7Eo4-unsplash.jpg')",
        }}
        role='img'
      ...
      <div className='max-w-6xl mx-auto p-4'>
        <br />
        <br />
        <div className='flex flex-col md:flex-row gap-4 md:gap-6 mb-6 items-center md:justify-center'>
          <Link
            to='/gear'
            aria-label='My Gear'
        ...
        {items.length === 0 ? (
          <div className='p-8 border-dashed border-2 border-gray-200 rounded text-center text-gray-600'>
            Your inventory is empty! Tap Add item to add your first piece of
            gear.
          </div>
        ) : (
                  ...
                  <>
                    <div>
                      ...
                      <div className='text-sm text-gray-600'>
                        ...
                        {it.boughtAt ? `• bought at ${it.boughtAt}` : ""}
                      </div>
                    </div>
